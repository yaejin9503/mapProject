...
export default function KakaoMap(props: IPropsMap) {
  ...
  const [optionsState, setOptionsState] =
    useState<kakao.maps.MapOptions | null>(null);
  ...
  useEffect(() => {
    if (containerState != null && optionsState != null) {
      ...
      setMap(
        new kakao.maps.Map(
          containerState as HTMLDivElement,
          optionsState as kakao.maps.MapOptions
        )
      ); //지도 생성 및 객체 리턴
    }
  ...
  useEffect(() => {
    const moveLatLon = new kakao.maps.LatLng(props.latitude, props.longitude);

    // 지도 중심을 이동 시킵니다
    map?.panTo(moveLatLon);
  }, [props.longitude, props.latitude, map]);
